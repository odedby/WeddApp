...
function decode(input) {
	 ...
	 function PostToPicasa(imageData){
	  var i = "papo";
	 alert(i);
	 alert("{'imgData':'{\"inPhoneNumber\":"+ decode(imageData)+"}'}");
		try{
		$.ajax({
			type: "POST",
			url: "http://papo84.no-ip.org/WeddApp/WeddAppService.asmx/UploadPhoto",
			data: "{'imgData':'{\"inPhoneNumber\":"+ imageData+"}'}",
	...
	function ShowLoader(pageName){
		...
		/*$('#page' + pageName).css('display','none');
	...
	function onPhotoURISuccess(imageURI) {
		...
		ft.upload(imageURI, "http://papo84.no-ip.org/WeddApp/upload.php", function(r) {
					//document.getElementById('camera_status').innerHTML = "Upload succeeded: "+r.bytesSent+" bytes uploaded.";
					HideLoader('Photos');
	 ...
	 xhr.onreadystatechange = function ()
	 {
 	 if(xhr.readyState == 4)
		 alert(xhr.status);
	 	 ...
	 	 alert("2");
	 xhr.open("POST","https://picasaweb.google.com/data/feed/api/user/104483653801439902987/albumid/5780002529047522017/default", true);
	 var type = "image/jpeg";
	 xhr.setRequestHeader('Accept','message/x-jl-formresult');
 	 xhr.setRequestHeader("content-type",type);
	 xhr.setRequestHeader('Content-Length',imageURI.size);
	 xhr.sendAsBinary(imageURI.getAsBinary());
	 /*
	 	 var myService = new PicasawebService("WeddApp");
	 myService.setUserCredentials("hagit.oded@gmail.com", "0542686874");
alert("3");
	 var albumPostUrl = new URL("https://picasaweb.google.com/data/feed/api/user/104483653801439902987/albumid/5780002529047522017");
alert("4");
	 ...
	 var myMedia = new MediaFileSource(new File("/home/liz/puppies.jpg"), "image/jpeg");
	 myPhoto.setMediaSource(myMedia);

	 var returnedPhoto = myService.insert(albumPostUrl, myPhoto);
	 
	 */
	 //Fixes from internet to code above from Google
	 //tring imgStrBase64 = req.getParameter("photo").split(",")[1];          
	...
	//Controling page divs visibility
	function showPic(url) {
	$('#Home').attr('onclick','Back()');
	$("#container").css("display","none");
	$("#divRefresh").css("display","none");
	$("#divPhoto").css("display","inline");
	$('#divPhoto').css("background-image", "url('" + url+ "')");  
	$('#imgPhoto').attr("src", url );  
	}
	function Back() {
	$('#Home').attr('onclick','Redirect("index")');
	$("#container").css("display","inline");
	$("#divRefresh").css("display","inline");
	$("#divPhoto").css("display","none");
	}

	//Getting Picasa Images
		...
		$.fn.pwi = function (opts) {
			...
			function photo(j, hidden, username) {
				...
				else
				{
					...
					if (($youtubeId == "") || (settings.popupPlugin === "slimbox")) {
						$html.append("<a onclick=showPic('" + $thumbnail1.url + "')><img src='" + $thumbnail0.url + "' height='" + $thumbnail0.height +
			...
			function album(j) {
				...
				if (settings.showAlbumDescription) {
					$scPhotosDesc = $("<div class='pwi_album_description' style='display:none;'/>");
